...
use sui_types::base_types::{FullObjectID, ObjectID};
...
use sui_types::storage::{BackingPackageStore, FullObjectKey, InputKey, MarkerValue};
                    ...
                    match self.get_object(&kind.object_id()) {
                        ...
                        _ => {
                            ...
                            if let Some((version, digest)) =
                                self.get_last_consensus_stream_end_info(input_full_id, epoch_id)
                            {
                                input_results[i] = Some(ObjectReadResult {
                                    input_object_kind: *kind,
...
impl ObjectCacheRead for InputLoaderCache<'_> {
    ...
    fn multi_input_objects_available_cache_only(&self, keys: &[InputKey]) -> Vec<bool> {
        self.loader
            .cache
            .multi_input_objects_available_cache_only(keys)
    }
